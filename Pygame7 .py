...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.surf = pygame.image.load("Asteriod.png").convert()
        self.surf.set_colorkey((255,255,255), RLEACCEL)
        self.rect = self.surf.get_rect(
            center = (
...
while running:
    ...
    # Check to see if enemy touching player
    if pygame.sprite.spritecollideany(player,enemies):
        #remove player and stop game
        player.kill()
        running = False
    ...
    if pygame.sprite.spritecollideany(player, enemies):
        # remove the player, and stop the main loop
        player.kill()
        running = False
